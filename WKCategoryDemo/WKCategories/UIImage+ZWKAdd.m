...
+ (UIImage *)qrCodeImageWithInfo:(NSString *)info centerImage:(UIImage *)centerImage  width:(CGFloat)width
{
    ...
    if (centerImage) {
        //icon尺寸,UIBezierPath
        CGSize logoSize = CGSizeMake(rect.size.width*0.25, rect.size.height*0.25);
        ...
        [[UIBezierPath bezierPathWithRoundedRect:logoFrame cornerRadius:15] addClip];
...
- (NSData *)compressionImageQualityWithmaxDataLength:(NSInteger)maxDataLength{
    ...
    if (maxDataLength > 0) {
        while (imageData.length > maxDataLength && compressionQuality > 0.05) {
